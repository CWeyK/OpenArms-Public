...
//fetch campaigns donated
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $campaignquery = "SELECT * FROM campaign WHERE campaignid IN (SELECT campaignid FROM donation WHERE donorid='" . $_SESSION['userid'] . "') AND (campaignid LIKE '%$search%' OR campaignName LIKE '%$search%' OR campaignDetails LIKE '%$search%' OR country LIKE '%$search%' OR category LIKE '%$search%' or state LIKE '%$search%')";
} else {
    $campaignquery = "SELECT * FROM campaign WHERE campaignid IN (SELECT campaignid FROM donation WHERE donorid='" . $_SESSION['userid'] . "')";
}
    ...
    <div class="mainContent">
        <div class="marginContent mt-4">
            <div class="d-flex justify-content-between">
                ...
                <div class="d-flex">
                    <a href="campaignOrganized.php" class="buttonGroup buttonGroup1">Organized</a>
                    <a href="campaignDonated.php" class="buttonGroup buttonGroup2 active">Donated</a>
                    <!-- search -->
                    <form action="campaignDonated.php" method="GET">
                    <div class="input-group mx-3" style="height: 30px; width: 250px;">
                        <input type="text" name="search" class="form-control form-control-sm" placeholder="Search...">
                        <?php if (isset($_GET['search'])) { ?>
                            <a href="campaignDonated.php" class="btn btn-outline-secondary py-1">Clear</a>
                        <?php } ?>
                    </div>
                    </form>
                </div>
                ...
                <?php 
                    ...
                    $raisedquery = "SELECT SUM(amount) AS total FROM donation WHERE campaignid=" . $campaign['campaignid'];
                        ...
                        if($campaign['approvalStatus']=='Approved'){
                            $campaign['approvalStatus']='Ongoing';
                        }
                ?>
                <div class="col-3">
                    ...
                    <div class="card p-0 my-3" style="width: 18rem;">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <!-- category -->
                                <p class="card-text cardCategory fw-semibold mb-0" style="font-size: 0.875rem;"><?=$campaign['category']?></p>
